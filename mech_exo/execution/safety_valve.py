...
from ..utils.structured_logging import ExecutionContext, ExecutionLogger
from .broker_adapter import BrokerAdapter
from .models import Order, OrderType, create_limit_order
...
class SafetyValve:
    ...
    def __init__(self, broker: BrokerAdapter, config: dict[str, Any] | None = None) -> None:
        self.broker = broker
    ...
    async def authorize_live_trading(self, session_description: str = "trading session") -> bool:
        """
        Authorize live trading with comprehensive safety checks.
        
        This method implements the safety valve protocol for live trading authorization:
        1. Verifies live trading mode is active
        2. Checks for emergency abort status
        3. Performs CLI confirmation (if configured)
        4. Executes sentinel order verification (if configured)
        
        Args:
            session_description: Description of the trading session for logging
            
        Returns:
            bool: True if all safety checks pass and live trading is authorized,
                 False if any safety check fails or user aborts
                 
        Raises:
            Exception: If safety valve initialization or verification fails
        """
        try:
    ...
    async def _get_cli_confirmation(self, session_description: str) -> bool:
        """
        Get interactive CLI confirmation from user for live trading.
        
        Displays comprehensive trading session information and requires
        explicit "yes" confirmation to proceed with live trading.
        
        Args:
            session_description: Description of the trading session
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if response in ["no", "n", "abort", "cancel", "exit"]:
                        logger.warning("❌ Live trading aborted by user")
                        return False
    ...
    async def _verify_sentinel_order(self) -> bool:
        """
        Verify broker connectivity and execution capability with a small sentinel order.
        
        Sends a small, low-risk order (typically CAD currency) to verify:
        - Broker connection is active
        - Order submission and processing works
        - Market access is functional
        
        Returns:
            bool: True if sentinel order completes successfully (filled or cancelled),
                 False if order fails, is rejected, or times out
        """
        try:
    ...
    async def check_order_safety(self, order: Order) -> dict[str, Any]:
        ...
        - Emergency abort status check
        - Daily value limit verification
        - Order size reasonableness check
        - Market hours validation
        
        Args:
            order: Order to validate for safety
            
        Returns:
            Dict containing:
                - 'approved' (bool): Whether order is safe to execute
                - 'reason' (str): Explanation for approval/rejection
                - 'warnings' (List[str]): Any safety warnings
                - 'order_value' (float): Calculated order value
                - 'daily_total' (float): Updated daily total if approved
        """
        try:
            warnings = []
    ...
    def activate_emergency_abort(self, reason: str = "Manual activation") -> None:
    ...
    def deactivate_emergency_abort(self, reason: str = "Manual deactivation") -> None:
